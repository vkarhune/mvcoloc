#' Simulate phenotypes, null associations (NOTE: not needed)
#'
#' @param seed
...
#'
#' @examples
simulate_phenotypes_null <- function(seed, genotype_data1, genotype_data2,
  ...
  while(!(cond)){
    ...
    if(length(candidates.2nd) > 1) cond <- TRUE
  }
  ...
  third.causal <- sample(setdiff(candidates.2nd, second.causal), 1, replace = F)

  cov.beta=cov(genotype_data2[,c(first.causal,second.causal,third.causal)])
  ...
  y = theta.vec[1] * genotype_data2[,first.causal] +
    ...
    theta.vec[3] * genotype_data2[,third.causal] +
  ...
  list(x1 = x1, x2 = x2, y = y, y.bin = y.bin, causals = c(first.causal, second.causal, third.causal))
